...
SELECT 'FIX A - bill total computed once per bill' AS FIX,
  ...
  FROM USER_SOURCE
 WHERE NAME = 'PKG_FIN_SUPPLIER_REPORTS' AND TYPE = 'PACKAGE BODY'
...
BEGIN
    ...
    PKG_FIN_SUPPLIER_REPORTS.SP_GET_SUPPLIER_PAYMENT_FORECAST (
        P_BNAME              => 'ALL',
        P_BRANCHID           => 1,
        P_PCODE              => 'ALL',
        P_VNAME              => 'ALL',
...
--      That is the whole explanation. Run PKG_FIN_SUPPLIER_REPORTS.sql in THIS
