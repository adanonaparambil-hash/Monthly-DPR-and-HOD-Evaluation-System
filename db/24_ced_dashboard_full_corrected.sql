------------------------------------------------------------------------------
-- 24_ced_dashboard_full_corrected.sql
-- COMPLETE, CORRECTED SP_GET_CED_DPR_DASHBOARD.
--
-- Paste this whole procedure into the PKG_TMST_TASK package BODY, replacing
-- the existing SP_GET_CED_DPR_DASHBOARD, then compile the package.
--
-- This file was generated FROM THE LIVE DEPLOYED PROCEDURE (fetched from
-- USER_SOURCE on 29-Jul-2026), so every line you already had is preserved
-- exactly. Only ONE change was applied, marked ## CAP in stage A3:
--
--   Indian leave days are now grouped per DATE and each date capped at 1.0,
--   matching PKG_DPR_CALC.FN_LEAVE_DAYS. The deployed version summed the raw
--   NOOFLEAVE rows, so a date carrying overlapping applications subtracted
--   more than one working day (live example: AIS62 on 28-Jun-2017 gave 2.0
--   instead of 1.0) and the dashboard's target drifted below the AI report's.
--
-- ALREADY CORRECT AND UNCHANGED (verified against PKG_DPR_CALC on live data
-- for 1-28 Jul 2026 — all three groups matched exactly):
--   * daily hours          Omani/Nationals 8, India 8, expat staff 9
--   * working days         Omani 20 (Fri+Sat off) / expat 24 (Fri off) /
--                          India 24 (Fri off, Sundays and 2nd/4th Saturdays
--                          ARE working days)
--   * leave                Oman = ADK2026 spans on that group's calendar;
--                          India = ADKIND_UAT per-day rows, Privilege Leave
--                          excluded, counted only on Indian working days
--   * HOD exclusion, IS_DPR='Y', CURRENTSTATUS='ACTIVE', DOJ clamping
--   * per-day daily/heatmap expectations and per-group 30-day consistency
--
-- KNOWN APPROXIMATION (unchanged, by design): a half-day Indian leave is
-- written to GT_DPR_LEAVE as a full 1-day span, so the daily-trend, heatmap
-- and 30-day-consistency views treat it as a whole day off. Expected hours
-- and compliance use the exact 0.5.
------------------------------------------------------------------------------
            ...
            UPDATE GT_DPR_EMP g
               SET g.LEAVE_DAYS =
                   CASE WHEN UPPER(TRIM(g.COM_LOC)) = 'IND' THEN               -- ## IND
                       -- ## CAP — group to ONE row per leave DATE and cap it at 1.0,
                       -- exactly like PKG_DPR_CALC.FN_LEAVE_DAYS. Without this,
                       -- two overlapping applications on the same date subtract
                       -- more than a single working day and the dashboard's
                       -- target drifts below the AI report's.
...
--
-- 1) The dashboard runs and returns data:
--
--    VAR s REFCURSOR
--    VAR d REFCURSOR
--    VAR t REFCURSOR
--    VAR h REFCURSOR
--    VAR e REFCURSOR
--    DECLARE ok VARCHAR2(1); msg VARCHAR2(500);
--    BEGIN
--      PKG_TMST_TASK.SP_GET_CED_DPR_DASHBOARD(
--        DATE '2026-07-01', DATE '2026-07-28', :s, :d, :t, :h, :e, ok, msg);
--      DBMS_OUTPUT.PUT_LINE(ok || ' - ' || msg);
--    END;
--    /
--    PRINT s
--
-- 2) Working days still agree with the AI section (expect 20 / 24 / 24):
--
--    SELECT PKG_DPR_CALC.FN_WORKING_DAYS(DATE '2026-07-01', DATE '2026-07-28', 'OM',  NULL, 'OMANI') AS OMANI,
--           PKG_DPR_CALC.FN_WORKING_DAYS(DATE '2026-07-01', DATE '2026-07-28', 'OM',  NULL, 'STAFF') AS EXPAT,
--           PKG_DPR_CALC.FN_WORKING_DAYS(DATE '2026-07-01', DATE '2026-07-28', 'IND', NULL, 'STAFF') AS INDIA
--      FROM DUAL;
--
-- 3) The ## CAP fix agrees with PKG_DPR_CALC for every Indian employee
--    (0 rows returned = the dashboard and the AI report subtract the same
--    leave; verified 0 mismatches across all 51 Indian DPR employees):
--
--    WITH DASH AS (
--      SELECT e.EMPID,
--             NVL((SELECT SUM(DAY_LEAVE)
