------------------------------------------------------------------------------
-- 14_ced_dashboard_india.sql — SP_GET_CED_DPR_DASHBOARD, India changes.
-- Paste this PROCEDURE over the existing one inside PKG_TMST_TASK body.
-- Based on the currently deployed version (HOD-exclusion + leave-aware);
-- every change is marked -- ## IND. Everyone except Indian staff behaves
-- exactly as before.
--
--   1. DAILY_EXP: Indian staff 8.5 -> 8 hours/day.
--   2. Indian leave sourced from ADKIND_UAT per-day rows (LEAVEDETAIL),
--      NOT from the Oman ADK2026.LBASIC spans:
--        * 'Privilege / Earned Leave' EXCLUDED — keeps full target hours.
--        * NOOFLEAVE 0.5 = half day -> LEAVE_DAYS is fractional for IND,
--          so EXP_HOURS drops by 4h for a half day, not 8h.
--      GT_DPR_LEAVE still receives one row per Indian leave DAY so the
--      daily-trend / heatmap / 30-day-consistency exclusions keep working
--      (a half-day counts as a whole day ONLY in those per-day displays;
--      the employee's expected hours use the exact fractional value).
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
-- VERIFY (after compiling BOTH 13 and this):
-- 1. An Indian employee's row in the employee cursor: DailyExpectedHrs = 8.
-- 2. RAHUL JAYARAM (AIS462), March 2026: LeaveDays = 1.5 (CL 1 + SL 0.5;
--    the three PL rows totalling 2.0 are NOT counted), ExpectedHrs drops
--    by 12h (1.5 x 8), not by 28 (3.5 x 8).
-- 3. An Omani employee's numbers are IDENTICAL to before this deploy.
--
-- Raw check of the Indian source, one employee:
-- SELECT e.EMPID, ld.LDATE, ld.NOOFLEAVE, lm.LEAVECODE, lm.LEAVEDESC,
--        CASE WHEN NVL(UPPER(lm.LEAVEDESC),' ') LIKE '%PRIVILEGE%'
--             THEN 'IGNORED (PL)' ELSE 'COUNTED' END AS TREATMENT
--   FROM ADKIND_UAT.LEAVEDETAIL ld
--   JOIN ADKIND_UAT.LBASIC  l  ON l.LBASICID  = ld.LBASICID AND l.CANCEL='F'
--   JOIN ADKIND_UAT.EMPMAST e  ON e.EMPMASTID = l.EMPMASTERID
--   JOIN ADKIND_UAT.LMDETAIL lm ON lm.LMDETAILID = ld.LVCODE
--  WHERE e.EMPID = 'AIS462'
--    AND ld.LDATE >= DATE '2026-03-01' AND ld.LDATE < DATE '2026-04-01'
--  ORDER BY ld.LDATE;
------------------------------------------------------------------------------
